...
Finally you will learn how to work with a special kind of vector called a factor, specifically designed to handle categorical values.
...
This introductory tutorial about the base R syntax can be used as a presentation by an instructor or as a self-study guide. It contains explanations, fully functional code boxes, and quizzes that make it very interactive. If time allows during a guided session, the student can execute code to prod and test how the syntax works providing invaluable hands-on feedback. The instructor may also ask short questions regarding the material. Should you fall behind or need more time, please do so after the live session. Actually this is very important in order to cement the concepts and test your understanding.
...
Did you notice the language used to explain subsetting? `x[1]` returns a vector with the first element of `x`. In R the subsetting function returns <span style="color:blue">another vector</span>. Let's verify these statements with R itself:
...
```{r examples-named-vectors1, exercise=TRUE, exercise.lines=10}
x <- c(5, -1, 10)
...
```{r multiplication-vectors-different-length-1, exercise=TRUE, exercise.lines=4}
...
The multiplication operator was applied element-wise as expected. In Section 4 of this workshop we will enhance this example to use matrices and lists. 
...
```{r element-wise-vector-multiplication, exercise=TRUE, exercise.lines=3, exercise.setup="setup-element-wise-vectors"}
...
```{r multiplication-vectors-different-length-2, exercise=TRUE, exercise.lines=2, exercise.setup="setup-element-wise-vectors"}
weekly_values
...
```{r clipping-recycled-vector-to-fit, exercise=TRUE, exercise.lines=3, warning=F, exercise.setup="setup-element-wise-vectors"}
...
Subsetting can be done by position or by value, @BaseR-Cheatsheet. Subsetting is akin to retaining some elements of a vector while leaving the rest out, kind of applying a filter to retain those elements.
...
```{r subsetting-a-vector, exercise=TRUE, exercise.lines=3}
x <- c(5, -1, 3, 0, -4, 1)
...
```{r, subsetting-vectors-with-vectos-of-positions, exercise=TRUE, exercise.lines=4, exercise.setup="setup-vector-to-subset-examples"}
...
```{r, subsetting-vectors-with-logical-masks, exercise=TRUE, exercise.lines=10, exercise.setup="setup-vector-to-subset-examples"}
...
```{r, subsetting-by-value-with-in-operator, exercise=TRUE, exercise.lines=7, exercise.setup="setup-vector-to-subset-examples"}
x
...
```{r, subsetting-by-value-with-element-name, exercise=TRUE, exercise.lines=2}
...
```{r recycling-a-logical-mask, exercise=TRUE, exercise.lines=3, exercise.setup="setup-vector-to-subset-examples"}
...
```{r equal-operator-on-vectors, exercise=TRUE, exercise.setup="equalonvectorssetup", exercise.lines=9}
...
```{r ampersand-on-vectors, exercise=TRUE, exercise.setup="ampersandonvectorssetup", exercise.lines=1}
week_days[gt_mean_mornings | gt_mean_afternoons]
```
...
<!-- ```{r double-ampersand-on-vectors, exercise=TRUE, exercise.setup="double-ampersand-on-vectors-setup", exercise.lines=1} -->
...
<!-- ```{r double-pipe-on-vectors, exercise=TRUE, exercise.setup="double-ampersand-on-vectors-setup", exercise.lines=1} -->
<!-- gt_mean_mornings || gt_mean_afternoons -->
<!-- ``` -->
...
```{r vector-coersion-example, exercise=TRUE, exercise.lines=3}
...
```{r type-of-NAs, exercise=TRUE, exercise.lines=5}
...
Some missing values are introduced by R code, for example trying to execute an illegal mathematical operation.


```{r type-of-NaNs, exercise=TRUE, exercise.lines=4}
35/0
...
The other common case for `NA` creation happens when sub-setting by a position that is out of the range of positions in the vector.
...
```{r, example-effect-of-NA-on-sum, exercise=TRUE, exercise.lines=8}
set.seed(3679)
v <- sample(1:100, 10)
...
```{r vectors-class, exercise=TRUE, exercise.lines=10}
...
```{r music-sample-genres, exercise=TRUE, exercise.lines=5}
...
```{r, music-sample-genres-as-factors, exercise=TRUE, exercise.lines=2, exercise.setup="setup-music-sample-genres"}
...
```{r, music-sample-adding_new-level, exercise=TRUE, exercise.lines=4, exercise.setup="setup-music-sample-genres"}
...
```{r, music-sample-genres-levels, exercise=TRUE, exercise.lines=4, exercise.setup="setup-music-sample-genres"}
# add new entries
