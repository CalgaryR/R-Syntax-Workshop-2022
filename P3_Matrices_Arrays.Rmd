...
```{r, 2by2by3array, exercise=TRUE, exercise.lines=8}
set.seed(673912)
( a <- array(sample.int(64,size = 12,replace = TRUE), dim = c(2,2,3)) )
```

Now let's build a logical mask to nullify the second frame and apply it to the original array.

```{r, logical-mask-for-an-array, exercise=TRUE, exercise.lines=8}
no_green_mask <- array(c(rep(1,4), rep(0,4), c(1,4)), dim = c(2,2,3))
( a * no_green_mask )
```
