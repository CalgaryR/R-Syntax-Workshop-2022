...
You will absolutely need to write a custom function whenever there is no other built-in or otherwise function available in a publicly available R package that meets your requirements.

Our main goal with this tutorial is to teach patterns of the R language basic syntax. 
...
```{r negation-logical-operator-complex-exp, exercise=TRUE, exercise.lines=3}
!(-4 > 0  & 0 > 1)
...
```{r negation-precedence, exercise=TRUE, exercise.lines=3}
!-4 > 0  & 0 > 1
```
...
R has several mechanisms to provide helpful information about the the language and the its modules, called libraries. 
For instance, to generate the help on operator precedence you can type `?Syntax` or `help("Syntax")` at the command line.

Run de code below to see a complete list of help commands available in any R installation.
...
R is a strongly typed language. To find out the type of R variables one can use the built-in functions `typeof()` and `mode()`. Let's test the former.

```{r type-of-variables, exercise=TRUE, exercise.lines=10}
b <- "day" == "night"
typeof(b)
i <- 54
typeof(i)
...
Remember how R uses coercion to transform a variable to the type expected by a logical operator like *NOT*?
If we changed the value of `a` above to a double or an integer, and applied the negation logical operator to it it would use coercion rules built into the language to provide a result instead of failing.
The rule in this case is a zero is interpreted as `FALSE` while non-zero is interpreted as `TRUE`.

We will discuss other coercion rules when talking about homogeneous data structures.
The homogeneous type assigned to all the elements when they are of heterogeneous types follow the rule of the most general type.
Decreasing in generality from left to right: `character` < `double` < `integer` < `logical`. 


## ✅️ Built-in functions

R offers predefined functions. When you use an R package you also use functions to get many complex tasks done with ease. It is important to familiarize yourself with their syntax. This requires that we start simple and build up the concepts gradually. The goal is to learn how to pass information to them and how to obtain results from them in two forms: as directly returned objects or as side effects. A side effect would be something like a file being written to the local disk or to a file service in the cloud.



## ✅️ Assignment from built-ins 
...
quiz(
  ...
  What is the result of executing the line above in the R console?",
    ...
    answer("nothing gets printed", correct = TRUE, message = "The operator _assignment_ has no side effects so nothing gets printed after the value 2 gets associated with the variable _a_"),
...
If the function is called without a second argument, and if `base` is required for computation inside the function, the assignment operator will use the value resulting from evaluating the expression `exp(1)` and associate it with `base` because that is how the built-in is defined. This is  how a default value for a named argument can be given. 
...
Now compare the flexibility and safety of using the explicit name assignment.

```{r named-param-3, exercise = TRUE}
# the first argument is now a named argument so the second gets position one instead
log(base = 10, 1000)
# in absence of hints, x = 10 (position one) and base = 1000 (anonymous argument)
log(10, 1000)
```

## ✅️ More on variable assignment
...
Did you get the same result of using `log(base = 10, 1000)`? That is almost true, the difference when using the global assingment operator, `<-`, is that an object with name `base` and value 10, gets created while calling `log` and continues to exist in the global environment after the function goes away.

In summary, to avoid ambiguities and unplanned side effects, use `<-` for stand-alone expressions and `=` for function named parameters. 
Also, be explicit when calling named parameters, it makes your code explicit in its intention and thus, easier to read for future you or others.
This is a fundamental feature of R syntax and it is not common in other data-oriented languages.





## ✅️ On number representation
...
## 📌 Tutorials &  online-resources
  ...
  4. https://masterr.org/r/RFP-part4-shadowing/ (advanced but necessary if writing big R programs)
