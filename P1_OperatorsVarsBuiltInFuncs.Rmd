...
Remember how R uses cohercion to trasform a variable to the type expected by a logical operator like *NOT*?
If we changed the value of `a` above to a double or an integer, and applied the negation logical operator to it it would use cohersion rules built into the language to provide a result instead of failing.
The rule in this case is a zero is interpreted as `FALSE` while non-zero is interpreted as `TRUE`.

We will discuss other cohercion rules when talking about homogeneous data structures.
