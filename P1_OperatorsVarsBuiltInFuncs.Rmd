...
!-4 > 0  & 0 > 1
```
As it turns out R coerces a value of `0` to `FALSE` and any non-zero to `TRUE` when in need to calculate using a logical operator, like `!` on -4. Thus the expression `!-4` translates into `!TRUE = FALSE` or `0`, which in turn makes `!-4 > 0` `FALSE`.
