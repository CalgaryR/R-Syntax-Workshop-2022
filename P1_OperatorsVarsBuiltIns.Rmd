...
!-4 > 0  & 0 > 1
```
As it turns out R coerces a value of `0` to `FALSE` or `F` and any non-zero to `TRUE` or `T` when in need of calculating the result of a logical operator, like `!` applied to a number like `-4`. Thus the expression `!-4` translates into `!TRUE = FALSE`, which in turn makes the relational operator `>` be faced with solving `F > 0`. The greater than operator, `>`, coerces the boolean and numeric values to characters to be able to process them, thus it compares their values on the ASCII table: `'F'=70` and `'0'=48`, therefore it computes `70 > 48 = TRUE`. The rest is similar, `'T' & 0 > 1` is reduced to `TRUE & FALSE` which is `FALSE`.
...
## ✅️ Finding the type of a variable
...
age <- 25; typeof(age)
# Uncomment the lines below one by one and check the result
#year <- 54L; typeof(year)

#number.pi <- 3.1416; typeof(number.pi)

#first.name <- "Rob"; typeof(first.name)

#is.Sunday <- ( weekdays(Sys.Date()) == "Sunday" )
#typeof(is.Sunday)
```
...
To read about an R package use the function call `library(help = "base")` at the command line. Packages are loaded by executing the function `libray("base")`, however this is not necessary for this package because it is loaded by default when an R session starts. Try the help about the package.
