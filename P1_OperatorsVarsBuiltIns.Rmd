...
Let's investigate the type of the R function `type` itself.
...
base = 100
log(1000, base = 10)
# What value does 'base' have in the global environment after log gest evaluated?
base
```
...
Why `log(base <- 10, 1000)` does not do the named argument assignment as when using `base=10`? The difference is that `base <- 10` creates a pair `(base, 10)` in the global environment but does not create a local variable `base`, it only passes the value of `10` in position 1.
