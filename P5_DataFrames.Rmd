...
set.seed(5529) # comment this line to see the impact on reproducibility
treatment <- LETTERS[sample(x = 1:26, size = 3, replace = FALSE)] # vector to be recycled by data frame
results <- round(runif(9), digits = 2) # equally distributed values between 0 and 1
passes_GT_0.7 <- results > 0.7
(experiments <- data.frame(treatment = treatment, 
                outcome = results,
                passes = passes_GT_0.7))
```
...
Then by counting the number of cases that pass our criteria (greater than `0.7` by the value of the column `passes`) in the data frame `num_successful_cases_by_treatment`.
...
```{r, aggregation-example-adding, exercise=TRUE, exercise.lines=8, exercise.setup="aggregationExamplesSetup"}
# first calculate the mean of val_1 by treatment
(outcome_mean_by_treatment <- aggregate(x = experiments$outcome,
                                   by = list(experiments$treatment),
                                   FUN = "mean"))
...
```{r, final-cbind, exercise=TRUE, exercise.lines=5, exercise.setup="aggregationExamplesSetup"}
# now put them together in a single data frame
aggregated_df <- cbind(outcome_mean_by_treatment,
                       num_successful_cases_by_treatment$x) # we pass the list 'x'
names(aggregated_df) <- c("treatment", "mean","# of cases>0.7")
aggregated_df
```

A second, arguably more elegant solution is to apply the R built-in function `merge` to join the two partial results for mean outcome and number of outcomes greater than 0.7.
...
```{r, merge, exercise=TRUE, exercise.lines=6, exercise.setup="aggregationExamplesSetup"}
results_df <- merge( x = outcome_mean_by_treatment, 
                     y = num_successful_cases_by_treatment, 
                     by = "Group.1", 
                     all = TRUE)
names(results_df) = c("treatment", "mean","# of cases>0.7")
results_df
```
