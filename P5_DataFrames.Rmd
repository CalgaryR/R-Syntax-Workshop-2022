...
In particular you will be able to leverage your knowledge of lists and vectors to create and manipulate data frames for powerful data analysis.
...
```{r slice-n-dice-df, exercise=T, exercise.setup="sliceAndDiceDfSetup", exercise.lines=9}
...
```{r subset-df-by-row, exercise=T, exercise.setup="subsetDfByRowSetup", exercise.lines=2}
df[df$country == "Canada",]
```

We can extract the countries with population greater than 80 million.

```{r subset-df-by-multiple-rows, exercise=T, exercise.setup="subsetDfByRowSetup", exercise.lines=4}
...
```{r explain-logical-mask-1, exercise=T, exercise.lines=1, exercise.setup="subsetDfByRowSetup"}
...
```{r explain-logical-mask-2, exercise=T, exercise.lines=2, exercise.setup="explainLogicalMaskSetup"}
...
```{r explain-logical-mask-3, exercise=T, exercise.lines=2, exercise.setup="explainLogicalMask3Setup"}
...
```{r, explore_J_and_J_data, exercise=T}
...
Now the matrix can be turned into a data frame. We could have also kept the matrix, can you answer why and what differences if any we would ahve encoutered? 
...
the names  for the two dimensions of the data frame. remember the `:` operator we learned as a shortcut for the `seq()` function in the section creating vectors with patterns?

```{R, constructAProperDataFrame, exercise=T}
(J_and_J <- data.frame( matrix(JohnsonJohnson, 
                              ncol=4, 
                              dimnames = list( 1960:1980, c("Qtr1", "Qtr2", "Qtr3", "Qtr4")))))
```
...
```{r, addColumnToDataFrame, exercise=T, exercise.setup="JAndJDataSetup"}
...
```{r, buildAMaskForJAndJDataFrame, exercise=T, exercise.setup="JAndJDataSetupWithTotals"}
...
```{r, finalSubsetJAndJDataFrame, exercise=T, exercise.setup="JAndJDataSetupWithTotals"}
subset(J_and_J, subset = mask.Q4.GT.60.pct.Total, select = c(4,5)) 
```

That was a company on a roll in the 70's!

**Note:** this function is one of the few that exposes a behaviour called non-standard evaluation in R. That means that when setting the arguments on a call to the function, the argument can be interpreted as a character string first if it exists. They are very convenient in interactive work at the command line (the REPL, [Read-evaluate-print-loop](https://en.wikipedia.org/wiki/Read%E2%80%93eval%E2%80%93print_loop), as it is known). In this scenario the user has in mind what he/she is interacting with, however the use of this function in a written script may lead to unintended interpretations of the arguments. The best practice is not to use `subset` in programs. The packages derived from the seminal work of Wickham [@tidy-data] provide consistent syntax to do these and many more operations on data.
...
```{r, aggregation-example-adding, exercise=T, exercise.lines=12, exercise.setup="aggregationExamplesSetup"}
...
```{r example-data-frame-strings-as-is, exercise=T, exercise.lines=2, exercise.setup="exampleDataFrameStringsAsIsSetup"}
...
```{r, createDataFrameWithNAsInfsAndNaNs, exercise=T}
set.seed(7777)
(experiments <- data.frame(experiment=rep(LETTERS[1:3], each=3), 
                  run=rep(1:3,3), 
                  results=round(runif(n = 9, min = 0.0, max = 10.0), digits = 2)))
...
```{r, detect-NAs-in-a-data-frame, exercise=T, exercise.setup="createDataFrameWithNAsInfsAndNaNsSetup"}
